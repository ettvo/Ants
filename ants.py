...
# pay attention to exits/entrances being None 
# p.entrance set to a non-None value when exit is constructed
# p.exit set when p is constructed
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        place = self.place
        distance = 0
        bee_target = bee_selector(place.bees)
        while distance < self.max_range and not bee_target:
            ...
            if distance >= self.min_range:
                bee_target = bee_selector(place.bees)
        if not bee_target or bee_target.place.is_hive(): 
            return None
        return bee_target
        

        """
        while not bee_target and i < self.max_range:
            if not path_list[i].entrance:
                return None
            path_list.append(path_list[i].entrance)
            i += 1
            if i >= self.min_range:
                bee_target = bee_selector(path_list[i].bees)
        if not bee_target or bee_target.place.is_hive():
            return None
        return bee_target
        """
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        for bee in list(self.place.bees):
            ...
            if bee.place and (self.health - amount) <= 0:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        """
        if not self.chewing:
            bee_target = bee_selector(self.place.bees)
            if bee_target:
                self.chewing = self.chew_duration
                bee_target.reduce_health(bee_target.health)
        else:
            self.chewing -= 1
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        ...
        for bee_target in list(self.place.bees):
            bee_target.reduce_health(self.damage)
...
class QueenAnt(ScubaThrower):
    ...
    def __init__(self, health=1):
        super().__init__(health)
        if not QueenAnt.existing_queen:
    ...
    def action(self, gamestate):
        ...
        while current_location:
            if current_location.ant:
                if current_location.ant.is_container() and current_location.ant.contained_ant:
                    current_location.ant.contained_ant.buff()
                current_location.ant.buff()
